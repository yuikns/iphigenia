...
object BasicWalkThrough {
  def main(args: Array[String]): Unit = {
    val trainMax = new DMatrix(XGBoostConf.trainFilePath)
    val testMax = new DMatrix(XGBoostConf.testFilePath)
    ...
    val spData = DataLoader.loadSVMFile(XGBoostConf.trainFilePath)
    val trainMax2 = new DMatrix(spData.rowHeaders, spData.colIndex, spData.data,
      JDMatrix.SparseType.CSR, 126)
